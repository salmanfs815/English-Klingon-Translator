...
int populate(BinarySearchTree<Entry>& bst, const string& filename)
{
	...
	{
		int elementsInserted = 0; //initialize elements count to 0
		...
		while ( getline (myfile,line) )
		{
			...
			if(bst.search(newEntry) == nullptr) // make sure that newEntry isn't already in the bst
				bst.add(newEntry); //add this entry to the binary search tree
			//cout << "new entry added to bst" << endl; // for debugging purposes
			elementsInserted++; //increment the elementsinserted variable
		}
		myfile.close(); //close the file
		//cout << "elements inserted: " << elementsInserted << endl; // for debugging purposes
...
Entry translate(BinarySearchTree<Entry>& bst, string word) //translate Entry from klingon to english
{
	BinaryNode<Entry>* translation = bst.search(Entry(word));
...
int main(int argc, char* argv[])
{
	...
	while(getline(cin,engWord))
	{
		...
		translations[i++] = translate(DictTree, engWord);
	}
